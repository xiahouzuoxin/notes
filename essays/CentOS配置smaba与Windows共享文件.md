...
使用samba需要穿过防火墙，因此配置/etc/sysconfig/iptables文件，添加
...
2.	新建smb用户用于访问Linux共享文件
	...
	此时/home目录会增加一个smb的用户。该Linux用户目录将可直接共享到Windows下。若要共享其它文件，按步骤3配置文件。
...
3.	samba配置文件。
	...
	path = /home/xiahouzuoxin/Public    # 共享的Linux目录
	public = no                         # 目录不公开
	writeable = yes                     # 可写
	browseable = yes                    # 可读
